...
class Node(object): #node class
    ...
    def perimeter(self):
        '''Calculates half the perimeter of the node's MBR.'''
    ...
    def min_dist_to_point(self, qx, qy):
        ...
        dx = max(self.MBR["x1"] - qx, 0, qx - self.MBR["x2"])
        dy = max(self.MBR["y1"] - qy, 0, qy - self.MBR["y2"])       
...
class RTree(object): #R tree class
    ...
    def insert(self, u, p): # insert p(data point) to u (/)
        '''
        Inserts a data point into the R-tree
        
        Args:
            u (Node): The node to insert the data point into.
            p (tuple): The data point to insert.
        '''
        if u.is_leaf(): 
    ...
    def choose_subtree(self, u, p): 
        '''
        Choose the best subtree to insert the data point to minimize the perimeter sum
        
        Args:
            u (Node): The current node to choose the subtree from.
            p (dict): The data point to be inserted.

        Returns:
            Node: The best subtree to insert the data point.
        '''
    ...
    def peri_increase(self, node, p):
        ...
        Args:
            node: The node to calculate the perimeter increase for.
            p: The data point to insert.
        
        Returns:
            The increase of the perimeter after inserting the new data point.
        '''
        # calculate the increase of the perimeter after inserting the new data point
    ...
    def handle_overflow(self, u):
        '''
        Handle overflow by splitting the node and updating the tree structure.
        
        Args:
            u (Node): The node that is overflowing.
        '''
    ...
    def split(self, u):
        ...
        Args:
            u (Node): The node to be split
        
        Returns:
            tuple: The two resulting nodes after splitting
        '''
    ...
    def add_child(self, node, child):
        '''Add a child node to the given node and update the MBRs.

        Args:
            node (Node): The node to which the child node will be added.
    ...
    def update_mbr(self, node): #update MBRs when forming a new MBR. It is used in checking the combinations and update the root
        '''
        Update the MBR of a node based on its child nodes or data points
        
        Args:
            node (Node): The node whose MBR needs to be updated
        '''
